...
Router.get("/",(req, res)=>{
    const data=[{name:"David",email:"john007@gmail.com"}];
    res.send(data);
});
...
Router.put("/api/updateuser/:id", (req, res)=>{
    ...
    Router.delete("/api/deleteuser/:id",(req,res)=>{
     dbconnected.query("DELETE FROM tbl_user where userid="+req.params.id+" ",(err,result)=>{
     if(!err){
        res.status(200).json({success:"User record deleted!"})
     }
     else {
        console.log(err);
    }
